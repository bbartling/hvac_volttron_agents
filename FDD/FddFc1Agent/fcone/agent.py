...
class Fcone(Agent):
    ...
    def check_list_size(self, arr):
        _log.debug(f'[FC 1 Agent INFO] - arr length is {len(arr)}')
        if len(arr) > 20:
            arr = arr[-20:]
            return arr
        else:
    ...
    def get_stuff_done(self):
        ...
        _log.debug(f'[FC 1 Agent INFO] - check_list_size check on duct_static_setpoint_data')

        if len(self.duct_static_setpoint_data) == 0:
            _log.debug(f'[FC 1 Agent INFO] - No data, not publishing anything or computing a fault')
            return

        else:        
            self.check_list_size(self.duct_static_setpoint_data)

            _log.debug(f'[FC 1 Agent INFO] - check_list_size check on duct_static_data')
            self.check_list_size(self.duct_static_data)

            _log.debug(f'[FC 1 Agent INFO] - check_list_size check on vfd_speed_data')
            self.check_list_size(self.vfd_speed_data)



            df = pd.DataFrame(list(zip(self.duct_static_data, 
                                        self.duct_static_setpoint_data,
                                        self.vfd_speed_data)), 
                                        columns =['duct_static',
                                        'duct_static_setpoint',
                                        'vfd_speed'])
            _log.debug(f'[FC 1 Agent INFO] - NOT rolling df is {df}')
            ...
            _log.debug(f'[FC 1 Agent INFO] - df last row is {df_lastrow}')
            ...
            self.vip.pubsub.publish(
                peer="pubsub",
                topic=f"FAULTS_FC1/{self.ahu_instance_id}",            
                headers={
